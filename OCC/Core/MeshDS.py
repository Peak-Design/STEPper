...
PYTHONOCC_DEBUG_EXCEPTIONS = _MeshDS.PYTHONOCC_DEBUG_EXCEPTIONS

def get_readable_class_name(class_name):
    return _MeshDS.get_readable_class_name(class_name)

def get_readable_method_name(method_name):
    return _MeshDS.get_readable_method_name(method_name)

def get_exception_type(error):
    return _MeshDS.get_exception_type(error)

def process_opencascade_exception(error, method_name, class_name):
    return _MeshDS.process_opencascade_exception(error, method_name, class_name)
...
def Handle_MeshDS_DataSource_ForceRelease(t):
    return _MeshDS.Handle_MeshDS_DataSource_ForceRelease(t)

def Handle_MeshDS_DataSource_GetRefCount(t):
    return _MeshDS.Handle_MeshDS_DataSource_GetRefCount(t)
