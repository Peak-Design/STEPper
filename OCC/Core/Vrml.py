# This file was automatically generated by SWIG (https://www.swig.org).
# Version 4.4.1
#
# Do not make changes to this file unless you know what you are doing - modify
# the SWIG interface file instead.

"""
Vrml module, see official documentation at
https://dev.opencascade.org/doc/occt-7.9.0/refman/html/package_vrml.html
"""

from sys import version_info as _swig_python_version_info
# Import the low-level C/C++ module
if getattr(globals().get("__spec__"), "parent", None) or __package__ or "." in __name__:
    from . import _Vrml
else:
    import _Vrml

try:
    import builtins as __builtin__
except ImportError:
    import __builtin__

def _swig_repr(self):
    try:
        strthis = "proxy of " + self.this.__repr__()
    except __builtin__.Exception:
        strthis = ""
    return "<%s.%s; %s >" % (self.__class__.__module__, self.__class__.__name__, strthis,)


def _swig_setattr_nondynamic_instance_variable(set):
    def set_instance_attr(self, name, value):
        if name == "this":
            set(self, name, value)
        elif name == "thisown":
            self.this.own(value)
        elif hasattr(self, name) and isinstance(getattr(type(self), name), property):
            set(self, name, value)
        else:
            raise AttributeError("You cannot add instance attributes to %s" % self)
    return set_instance_attr


def _swig_setattr_nondynamic_class_variable(set):
    def set_class_attr(cls, name, value):
        if hasattr(cls, name) and not isinstance(getattr(cls, name), property):
            set(cls, name, value)
        else:
            raise AttributeError("You cannot add class attributes to %s" % cls)
    return set_class_attr


def _swig_add_metaclass(metaclass):
    """Class decorator for adding a metaclass to a SWIG wrapped class - a slimmed down version of six.add_metaclass"""
    def wrapper(cls):
        return metaclass(cls.__name__, cls.__bases__, cls.__dict__.copy())
    return wrapper


class _SwigNonDynamicMeta(type):
    """Meta class to enforce nondynamic attributes (no new attributes) for a class"""
    __setattr__ = _swig_setattr_nondynamic_class_variable(type.__setattr__)


class SwigPyIterator(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")

    def __init__(self, *args, **kwargs):
        raise AttributeError("No constructor defined - class is abstract")
    __repr__ = _swig_repr
    __swig_destroy__ = _Vrml.delete_SwigPyIterator

    def value(self):
        return _Vrml.SwigPyIterator_value(self)

    def incr(self, n=1):
        return _Vrml.SwigPyIterator_incr(self, n)

    def decr(self, n=1):
        return _Vrml.SwigPyIterator_decr(self, n)

    def distance(self, x):
        return _Vrml.SwigPyIterator_distance(self, x)

    def equal(self, x):
        return _Vrml.SwigPyIterator_equal(self, x)

    def copy(self):
        return _Vrml.SwigPyIterator_copy(self)

    def next(self):
        return _Vrml.SwigPyIterator_next(self)

    def __next__(self):
        return _Vrml.SwigPyIterator___next__(self)

    def previous(self):
        return _Vrml.SwigPyIterator_previous(self)

    def advance(self, n):
        return _Vrml.SwigPyIterator_advance(self, n)

    def __eq__(self, x):
        return _Vrml.SwigPyIterator___eq__(self, x)

    def __ne__(self, x):
        return _Vrml.SwigPyIterator___ne__(self, x)

    def __iadd__(self, n):
        return _Vrml.SwigPyIterator___iadd__(self, n)

    def __isub__(self, n):
        return _Vrml.SwigPyIterator___isub__(self, n)

    def __add__(self, n):
        return _Vrml.SwigPyIterator___add__(self, n)

    def __sub__(self, *args):
        return _Vrml.SwigPyIterator___sub__(self, *args)
    def __iter__(self):
        return self

# Register SwigPyIterator in _Vrml:
_Vrml.SwigPyIterator_swigregister(SwigPyIterator)

def _dumps_object(klass):
    """ Overwrite default string output for any wrapped object.
    By default, __repr__ method returns something like:
    <OCC.Core.TopoDS.TopoDS_Shape; proxy of <Swig Object of type 'TopoDS_Shape *' at 0x02BB0758> >
    This is too much verbose.
    We prefer :
    <class 'gp_Pnt'>
    or
    <class 'TopoDS_Shape'>
    """
    klass_name = str(klass.__class__).split(".")[3].split("'")[0]
    repr_string = "<class '" + klass_name + "'"
# for TopoDS_Shape, we also look for the base type
    if klass_name == "TopoDS_Shape":
        if klass.IsNull():
            repr_string += ": Null>"
            return repr_string
        st = klass.ShapeType()
        types = {OCC.Core.TopAbs.TopAbs_VERTEX: "Vertex",
                 OCC.Core.TopAbs.TopAbs_SOLID: "Solid",
                 OCC.Core.TopAbs.TopAbs_EDGE: "Edge",
                 OCC.Core.TopAbs.TopAbs_FACE: "Face",
                 OCC.Core.TopAbs.TopAbs_SHELL: "Shell",
                 OCC.Core.TopAbs.TopAbs_WIRE: "Wire",
                 OCC.Core.TopAbs.TopAbs_COMPOUND: "Compound",
                 OCC.Core.TopAbs.TopAbs_COMPSOLID: "Compsolid"}
        repr_string += "; Type:%s" % types[st]        
    elif hasattr(klass, "IsNull"):
        if klass.IsNull():
            repr_string += "; Null"
    repr_string += ">"
    return repr_string

PYTHONOCC_DEBUG_EXCEPTIONS = _Vrml.PYTHONOCC_DEBUG_EXCEPTIONS

def get_readable_class_name(class_name):
    return _Vrml.get_readable_class_name(class_name)

def get_readable_method_name(method_name):
    return _Vrml.get_readable_method_name(method_name)

def get_exception_type(error):
    return _Vrml.get_exception_type(error)

def process_opencascade_exception(error, method_name, class_name):
    return _Vrml.process_opencascade_exception(error, method_name, class_name)

import warnings
from OCC.Wrapper.wrapper_utils import Proxy, deprecated

class ios_base(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")

    def __init__(self, *args, **kwargs):
        raise AttributeError("No constructor defined")
    __repr__ = _swig_repr
    erase_event = _Vrml.ios_base_erase_event
    imbue_event = _Vrml.ios_base_imbue_event
    copyfmt_event = _Vrml.ios_base_copyfmt_event

    def register_callback(self, __fn, __index):
        return _Vrml.ios_base_register_callback(self, __fn, __index)

    def flags(self, *args):
        return _Vrml.ios_base_flags(self, *args)

    def setf(self, *args):
        return _Vrml.ios_base_setf(self, *args)

    def unsetf(self, __mask):
        return _Vrml.ios_base_unsetf(self, __mask)

    def precision(self, *args):
        return _Vrml.ios_base_precision(self, *args)

    def width(self, *args):
        return _Vrml.ios_base_width(self, *args)

    @staticmethod
    def sync_with_stdio(__sync=True):
        return _Vrml.ios_base_sync_with_stdio(__sync)

    def imbue(self, __loc):
        return _Vrml.ios_base_imbue(self, __loc)

    def getloc(self):
        return _Vrml.ios_base_getloc(self)

    @staticmethod
    def xalloc():
        return _Vrml.ios_base_xalloc()

    def iword(self, __ix):
        return _Vrml.ios_base_iword(self, __ix)

    def pword(self, __ix):
        return _Vrml.ios_base_pword(self, __ix)
    __swig_destroy__ = _Vrml.delete_ios_base

# Register ios_base in _Vrml:
_Vrml.ios_base_swigregister(ios_base)
cvar = _Vrml.cvar
ios_base.boolalpha = _Vrml.cvar.ios_base_boolalpha
ios_base.dec = _Vrml.cvar.ios_base_dec
ios_base.fixed = _Vrml.cvar.ios_base_fixed
ios_base.hex = _Vrml.cvar.ios_base_hex
ios_base.internal = _Vrml.cvar.ios_base_internal
ios_base.left = _Vrml.cvar.ios_base_left
ios_base.oct = _Vrml.cvar.ios_base_oct
ios_base.right = _Vrml.cvar.ios_base_right
ios_base.scientific = _Vrml.cvar.ios_base_scientific
ios_base.showbase = _Vrml.cvar.ios_base_showbase
ios_base.showpoint = _Vrml.cvar.ios_base_showpoint
ios_base.showpos = _Vrml.cvar.ios_base_showpos
ios_base.skipws = _Vrml.cvar.ios_base_skipws
ios_base.unitbuf = _Vrml.cvar.ios_base_unitbuf
ios_base.uppercase = _Vrml.cvar.ios_base_uppercase
ios_base.adjustfield = _Vrml.cvar.ios_base_adjustfield
ios_base.basefield = _Vrml.cvar.ios_base_basefield
ios_base.floatfield = _Vrml.cvar.ios_base_floatfield
ios_base.badbit = _Vrml.cvar.ios_base_badbit
ios_base.eofbit = _Vrml.cvar.ios_base_eofbit
ios_base.failbit = _Vrml.cvar.ios_base_failbit
ios_base.goodbit = _Vrml.cvar.ios_base_goodbit
ios_base.app = _Vrml.cvar.ios_base_app
ios_base.ate = _Vrml.cvar.ios_base_ate
ios_base.binary = _Vrml.cvar.ios_base_binary
ios_base.ios_base_in = _Vrml.cvar.ios_base_ios_base_in
ios_base.out = _Vrml.cvar.ios_base_out
ios_base.trunc = _Vrml.cvar.ios_base_trunc
ios_base.beg = _Vrml.cvar.ios_base_beg
ios_base.cur = _Vrml.cvar.ios_base_cur
ios_base.end = _Vrml.cvar.ios_base_end

class ios(ios_base):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def rdstate(self):
        return _Vrml.ios_rdstate(self)

    def clear(self, *args):
        return _Vrml.ios_clear(self, *args)

    def setstate(self, __state):
        return _Vrml.ios_setstate(self, __state)

    def good(self):
        return _Vrml.ios_good(self)

    def eof(self):
        return _Vrml.ios_eof(self)

    def fail(self):
        return _Vrml.ios_fail(self)

    def bad(self):
        return _Vrml.ios_bad(self)

    def exceptions(self, *args):
        return _Vrml.ios_exceptions(self, *args)

    def __init__(self, __sb):
        _Vrml.ios_swiginit(self, _Vrml.new_ios(__sb))
    __swig_destroy__ = _Vrml.delete_ios

    def tie(self, *args):
        return _Vrml.ios_tie(self, *args)

    def rdbuf(self, *args):
        return _Vrml.ios_rdbuf(self, *args)

    def copyfmt(self, __rhs):
        return _Vrml.ios_copyfmt(self, __rhs)

    def fill(self, *args):
        return _Vrml.ios_fill(self, *args)

    def imbue(self, __loc):
        return _Vrml.ios_imbue(self, __loc)

    def narrow(self, __c, __dfault):
        return _Vrml.ios_narrow(self, __c, __dfault)

    def widen(self, __c):
        return _Vrml.ios_widen(self, __c)

# Register ios in _Vrml:
_Vrml.ios_swigregister(ios)
class ostream(ios):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, __sb):
        _Vrml.ostream_swiginit(self, _Vrml.new_ostream(__sb))
    __swig_destroy__ = _Vrml.delete_ostream

    def __lshift__(self, *args):
        return _Vrml.ostream___lshift__(self, *args)

    def put(self, __c):
        return _Vrml.ostream_put(self, __c)

    def write(self, __s, __n):
        return _Vrml.ostream_write(self, __s, __n)

    def flush(self):
        return _Vrml.ostream_flush(self)

    def tellp(self):
        return _Vrml.ostream_tellp(self)

    def seekp(self, *args):
        return _Vrml.ostream_seekp(self, *args)

# Register ostream in _Vrml:
_Vrml.ostream_swigregister(ostream)
class istream(ios):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, __sb):
        _Vrml.istream_swiginit(self, _Vrml.new_istream(__sb))
    __swig_destroy__ = _Vrml.delete_istream

    def __rshift__(self, *args):
        return _Vrml.istream___rshift__(self, *args)

    def gcount(self):
        return _Vrml.istream_gcount(self)

    def get(self, *args):
        return _Vrml.istream_get(self, *args)

    def getline(self, *args):
        return _Vrml.istream_getline(self, *args)

    def ignore(self, *args):
        return _Vrml.istream_ignore(self, *args)

    def peek(self):
        return _Vrml.istream_peek(self)

    def read(self, __s, __n):
        return _Vrml.istream_read(self, __s, __n)

    def readsome(self, __s, __n):
        return _Vrml.istream_readsome(self, __s, __n)

    def putback(self, __c):
        return _Vrml.istream_putback(self, __c)

    def unget(self):
        return _Vrml.istream_unget(self)

    def sync(self):
        return _Vrml.istream_sync(self)

    def tellg(self):
        return _Vrml.istream_tellg(self)

    def seekg(self, *args):
        return _Vrml.istream_seekg(self, *args)

# Register istream in _Vrml:
_Vrml.istream_swigregister(istream)
class iostream(istream, ostream):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, __sb):
        _Vrml.iostream_swiginit(self, _Vrml.new_iostream(__sb))
    __swig_destroy__ = _Vrml.delete_iostream

# Register iostream in _Vrml:
_Vrml.iostream_swigregister(iostream)
cin = cvar.cin
cout = cvar.cout
cerr = cvar.cerr
clog = cvar.clog

endl_cb_ptr = _Vrml.endl_cb_ptr
endl = _Vrml.endl
ends_cb_ptr = _Vrml.ends_cb_ptr
ends = _Vrml.ends
flush_cb_ptr = _Vrml.flush_cb_ptr
flush = _Vrml.flush
import OCC.Core.Standard
import OCC.Core.NCollection
import OCC.Core.TColStd
import OCC.Core.TCollection
import OCC.Core.TColgp
import OCC.Core.Quantity
import OCC.Core.gp

from enum import IntEnum
from OCC.Core.Exception import *

Vrml_LEFT = _Vrml.Vrml_LEFT
Vrml_CENTER = _Vrml.Vrml_CENTER
Vrml_RIGHT = _Vrml.Vrml_RIGHT
Vrml_ConeSIDES = _Vrml.Vrml_ConeSIDES
Vrml_ConeBOTTOM = _Vrml.Vrml_ConeBOTTOM
Vrml_ConeALL = _Vrml.Vrml_ConeALL
Vrml_CylinderSIDES = _Vrml.Vrml_CylinderSIDES
Vrml_CylinderTOP = _Vrml.Vrml_CylinderTOP
Vrml_CylinderBOTTOM = _Vrml.Vrml_CylinderBOTTOM
Vrml_CylinderALL = _Vrml.Vrml_CylinderALL
Vrml_UNKNOWN_FACE_TYPE = _Vrml.Vrml_UNKNOWN_FACE_TYPE
Vrml_CONVEX = _Vrml.Vrml_CONVEX
Vrml_SERIF = _Vrml.Vrml_SERIF
Vrml_SANS = _Vrml.Vrml_SANS
Vrml_TYPEWRITER = _Vrml.Vrml_TYPEWRITER
Vrml_NONE = _Vrml.Vrml_NONE
Vrml_BOLD = _Vrml.Vrml_BOLD
Vrml_ITALIC = _Vrml.Vrml_ITALIC
Vrml_DEFAULT = _Vrml.Vrml_DEFAULT
Vrml_OVERALL = _Vrml.Vrml_OVERALL
Vrml_PER_PART = _Vrml.Vrml_PER_PART
Vrml_PER_PART_INDEXED = _Vrml.Vrml_PER_PART_INDEXED
Vrml_PER_FACE = _Vrml.Vrml_PER_FACE
Vrml_PER_FACE_INDEXED = _Vrml.Vrml_PER_FACE_INDEXED
Vrml_PER_VERTEX = _Vrml.Vrml_PER_VERTEX
Vrml_PER_VERTEX_INDEXED = _Vrml.Vrml_PER_VERTEX_INDEXED
Vrml_NULL = _Vrml.Vrml_NULL
Vrml_ONE = _Vrml.Vrml_ONE
Vrml_TWO = _Vrml.Vrml_TWO
Vrml_THREE = _Vrml.Vrml_THREE
Vrml_FOUR = _Vrml.Vrml_FOUR
Vrml_OFF = _Vrml.Vrml_OFF
Vrml_ON = _Vrml.Vrml_ON
Vrml_AUTO = _Vrml.Vrml_AUTO
Vrml_UNKNOWN_SHAPE_TYPE = _Vrml.Vrml_UNKNOWN_SHAPE_TYPE
Vrml_SOLID = _Vrml.Vrml_SOLID
Vrml_REPEAT = _Vrml.Vrml_REPEAT
Vrml_CLAMP = _Vrml.Vrml_CLAMP
Vrml_UNKNOWN_ORDERING = _Vrml.Vrml_UNKNOWN_ORDERING
Vrml_CLOCKWISE = _Vrml.Vrml_CLOCKWISE
Vrml_COUNTERCLOCKWISE = _Vrml.Vrml_COUNTERCLOCKWISE
Vrml_MAP_NONE = _Vrml.Vrml_MAP_NONE
Vrml_POINT = _Vrml.Vrml_POINT


class Vrml_AsciiTextJustification(IntEnum):
	Vrml_LEFT = 0
	Vrml_CENTER = 1
	Vrml_RIGHT = 2
Vrml_LEFT = Vrml_AsciiTextJustification.Vrml_LEFT
Vrml_CENTER = Vrml_AsciiTextJustification.Vrml_CENTER
Vrml_RIGHT = Vrml_AsciiTextJustification.Vrml_RIGHT

class Vrml_ConeParts(IntEnum):
	Vrml_ConeSIDES = 0
	Vrml_ConeBOTTOM = 1
	Vrml_ConeALL = 2
Vrml_ConeSIDES = Vrml_ConeParts.Vrml_ConeSIDES
Vrml_ConeBOTTOM = Vrml_ConeParts.Vrml_ConeBOTTOM
Vrml_ConeALL = Vrml_ConeParts.Vrml_ConeALL

class Vrml_CylinderParts(IntEnum):
	Vrml_CylinderSIDES = 0
	Vrml_CylinderTOP = 1
	Vrml_CylinderBOTTOM = 2
	Vrml_CylinderALL = 3
Vrml_CylinderSIDES = Vrml_CylinderParts.Vrml_CylinderSIDES
Vrml_CylinderTOP = Vrml_CylinderParts.Vrml_CylinderTOP
Vrml_CylinderBOTTOM = Vrml_CylinderParts.Vrml_CylinderBOTTOM
Vrml_CylinderALL = Vrml_CylinderParts.Vrml_CylinderALL

class Vrml_FaceType(IntEnum):
	Vrml_UNKNOWN_FACE_TYPE = 0
	Vrml_CONVEX = 1
Vrml_UNKNOWN_FACE_TYPE = Vrml_FaceType.Vrml_UNKNOWN_FACE_TYPE
Vrml_CONVEX = Vrml_FaceType.Vrml_CONVEX

class Vrml_FontStyleFamily(IntEnum):
	Vrml_SERIF = 0
	Vrml_SANS = 1
	Vrml_TYPEWRITER = 2
Vrml_SERIF = Vrml_FontStyleFamily.Vrml_SERIF
Vrml_SANS = Vrml_FontStyleFamily.Vrml_SANS
Vrml_TYPEWRITER = Vrml_FontStyleFamily.Vrml_TYPEWRITER

class Vrml_FontStyleStyle(IntEnum):
	Vrml_NONE = 0
	Vrml_BOLD = 1
	Vrml_ITALIC = 2
Vrml_NONE = Vrml_FontStyleStyle.Vrml_NONE
Vrml_BOLD = Vrml_FontStyleStyle.Vrml_BOLD
Vrml_ITALIC = Vrml_FontStyleStyle.Vrml_ITALIC

class Vrml_MaterialBindingAndNormalBinding(IntEnum):
	Vrml_DEFAULT = 0
	Vrml_OVERALL = 1
	Vrml_PER_PART = 2
	Vrml_PER_PART_INDEXED = 3
	Vrml_PER_FACE = 4
	Vrml_PER_FACE_INDEXED = 5
	Vrml_PER_VERTEX = 6
	Vrml_PER_VERTEX_INDEXED = 7
Vrml_DEFAULT = Vrml_MaterialBindingAndNormalBinding.Vrml_DEFAULT
Vrml_OVERALL = Vrml_MaterialBindingAndNormalBinding.Vrml_OVERALL
Vrml_PER_PART = Vrml_MaterialBindingAndNormalBinding.Vrml_PER_PART
Vrml_PER_PART_INDEXED = Vrml_MaterialBindingAndNormalBinding.Vrml_PER_PART_INDEXED
Vrml_PER_FACE = Vrml_MaterialBindingAndNormalBinding.Vrml_PER_FACE
Vrml_PER_FACE_INDEXED = Vrml_MaterialBindingAndNormalBinding.Vrml_PER_FACE_INDEXED
Vrml_PER_VERTEX = Vrml_MaterialBindingAndNormalBinding.Vrml_PER_VERTEX
Vrml_PER_VERTEX_INDEXED = Vrml_MaterialBindingAndNormalBinding.Vrml_PER_VERTEX_INDEXED

class Vrml_SFImageNumber(IntEnum):
	Vrml_NULL = 0
	Vrml_ONE = 1
	Vrml_TWO = 2
	Vrml_THREE = 3
	Vrml_FOUR = 4
Vrml_NULL = Vrml_SFImageNumber.Vrml_NULL
Vrml_ONE = Vrml_SFImageNumber.Vrml_ONE
Vrml_TWO = Vrml_SFImageNumber.Vrml_TWO
Vrml_THREE = Vrml_SFImageNumber.Vrml_THREE
Vrml_FOUR = Vrml_SFImageNumber.Vrml_FOUR

class Vrml_SeparatorRenderCulling(IntEnum):
	Vrml_OFF = 0
	Vrml_ON = 1
	Vrml_AUTO = 2
Vrml_OFF = Vrml_SeparatorRenderCulling.Vrml_OFF
Vrml_ON = Vrml_SeparatorRenderCulling.Vrml_ON
Vrml_AUTO = Vrml_SeparatorRenderCulling.Vrml_AUTO

class Vrml_ShapeType(IntEnum):
	Vrml_UNKNOWN_SHAPE_TYPE = 0
	Vrml_SOLID = 1
Vrml_UNKNOWN_SHAPE_TYPE = Vrml_ShapeType.Vrml_UNKNOWN_SHAPE_TYPE
Vrml_SOLID = Vrml_ShapeType.Vrml_SOLID

class Vrml_Texture2Wrap(IntEnum):
	Vrml_REPEAT = 0
	Vrml_CLAMP = 1
Vrml_REPEAT = Vrml_Texture2Wrap.Vrml_REPEAT
Vrml_CLAMP = Vrml_Texture2Wrap.Vrml_CLAMP

class Vrml_VertexOrdering(IntEnum):
	Vrml_UNKNOWN_ORDERING = 0
	Vrml_CLOCKWISE = 1
	Vrml_COUNTERCLOCKWISE = 2
Vrml_UNKNOWN_ORDERING = Vrml_VertexOrdering.Vrml_UNKNOWN_ORDERING
Vrml_CLOCKWISE = Vrml_VertexOrdering.Vrml_CLOCKWISE
Vrml_COUNTERCLOCKWISE = Vrml_VertexOrdering.Vrml_COUNTERCLOCKWISE

class Vrml_WWWAnchorMap(IntEnum):
	Vrml_MAP_NONE = 0
	Vrml_POINT = 1
Vrml_MAP_NONE = Vrml_WWWAnchorMap.Vrml_MAP_NONE
Vrml_POINT = Vrml_WWWAnchorMap.Vrml_POINT


def Handle_Vrml_AsciiText_Create():
    return _Vrml.Handle_Vrml_AsciiText_Create()

def Handle_Vrml_AsciiText_DownCast(t):
    return _Vrml.Handle_Vrml_AsciiText_DownCast(t)

def Handle_Vrml_AsciiText_IsNull(t):
    return _Vrml.Handle_Vrml_AsciiText_IsNull(t)

def Handle_Vrml_AsciiText_ForceRelease(t):
    return _Vrml.Handle_Vrml_AsciiText_ForceRelease(t)

def Handle_Vrml_AsciiText_GetRefCount(t):
    return _Vrml.Handle_Vrml_AsciiText_GetRefCount(t)

def Handle_Vrml_Coordinate3_Create():
    return _Vrml.Handle_Vrml_Coordinate3_Create()

def Handle_Vrml_Coordinate3_DownCast(t):
    return _Vrml.Handle_Vrml_Coordinate3_DownCast(t)

def Handle_Vrml_Coordinate3_IsNull(t):
    return _Vrml.Handle_Vrml_Coordinate3_IsNull(t)

def Handle_Vrml_Coordinate3_ForceRelease(t):
    return _Vrml.Handle_Vrml_Coordinate3_ForceRelease(t)

def Handle_Vrml_Coordinate3_GetRefCount(t):
    return _Vrml.Handle_Vrml_Coordinate3_GetRefCount(t)

def Handle_Vrml_IndexedFaceSet_Create():
    return _Vrml.Handle_Vrml_IndexedFaceSet_Create()

def Handle_Vrml_IndexedFaceSet_DownCast(t):
    return _Vrml.Handle_Vrml_IndexedFaceSet_DownCast(t)

def Handle_Vrml_IndexedFaceSet_IsNull(t):
    return _Vrml.Handle_Vrml_IndexedFaceSet_IsNull(t)

def Handle_Vrml_IndexedFaceSet_ForceRelease(t):
    return _Vrml.Handle_Vrml_IndexedFaceSet_ForceRelease(t)

def Handle_Vrml_IndexedFaceSet_GetRefCount(t):
    return _Vrml.Handle_Vrml_IndexedFaceSet_GetRefCount(t)

def Handle_Vrml_IndexedLineSet_Create():
    return _Vrml.Handle_Vrml_IndexedLineSet_Create()

def Handle_Vrml_IndexedLineSet_DownCast(t):
    return _Vrml.Handle_Vrml_IndexedLineSet_DownCast(t)

def Handle_Vrml_IndexedLineSet_IsNull(t):
    return _Vrml.Handle_Vrml_IndexedLineSet_IsNull(t)

def Handle_Vrml_IndexedLineSet_ForceRelease(t):
    return _Vrml.Handle_Vrml_IndexedLineSet_ForceRelease(t)

def Handle_Vrml_IndexedLineSet_GetRefCount(t):
    return _Vrml.Handle_Vrml_IndexedLineSet_GetRefCount(t)

def Handle_Vrml_LOD_Create():
    return _Vrml.Handle_Vrml_LOD_Create()

def Handle_Vrml_LOD_DownCast(t):
    return _Vrml.Handle_Vrml_LOD_DownCast(t)

def Handle_Vrml_LOD_IsNull(t):
    return _Vrml.Handle_Vrml_LOD_IsNull(t)

def Handle_Vrml_LOD_ForceRelease(t):
    return _Vrml.Handle_Vrml_LOD_ForceRelease(t)

def Handle_Vrml_LOD_GetRefCount(t):
    return _Vrml.Handle_Vrml_LOD_GetRefCount(t)

def Handle_Vrml_Material_Create():
    return _Vrml.Handle_Vrml_Material_Create()

def Handle_Vrml_Material_DownCast(t):
    return _Vrml.Handle_Vrml_Material_DownCast(t)

def Handle_Vrml_Material_IsNull(t):
    return _Vrml.Handle_Vrml_Material_IsNull(t)

def Handle_Vrml_Material_ForceRelease(t):
    return _Vrml.Handle_Vrml_Material_ForceRelease(t)

def Handle_Vrml_Material_GetRefCount(t):
    return _Vrml.Handle_Vrml_Material_GetRefCount(t)

def Handle_Vrml_Normal_Create():
    return _Vrml.Handle_Vrml_Normal_Create()

def Handle_Vrml_Normal_DownCast(t):
    return _Vrml.Handle_Vrml_Normal_DownCast(t)

def Handle_Vrml_Normal_IsNull(t):
    return _Vrml.Handle_Vrml_Normal_IsNull(t)

def Handle_Vrml_Normal_ForceRelease(t):
    return _Vrml.Handle_Vrml_Normal_ForceRelease(t)

def Handle_Vrml_Normal_GetRefCount(t):
    return _Vrml.Handle_Vrml_Normal_GetRefCount(t)

def Handle_Vrml_SFImage_Create():
    return _Vrml.Handle_Vrml_SFImage_Create()

def Handle_Vrml_SFImage_DownCast(t):
    return _Vrml.Handle_Vrml_SFImage_DownCast(t)

def Handle_Vrml_SFImage_IsNull(t):
    return _Vrml.Handle_Vrml_SFImage_IsNull(t)

def Handle_Vrml_SFImage_ForceRelease(t):
    return _Vrml.Handle_Vrml_SFImage_ForceRelease(t)

def Handle_Vrml_SFImage_GetRefCount(t):
    return _Vrml.Handle_Vrml_SFImage_GetRefCount(t)

def Handle_Vrml_TextureCoordinate2_Create():
    return _Vrml.Handle_Vrml_TextureCoordinate2_Create()

def Handle_Vrml_TextureCoordinate2_DownCast(t):
    return _Vrml.Handle_Vrml_TextureCoordinate2_DownCast(t)

def Handle_Vrml_TextureCoordinate2_IsNull(t):
    return _Vrml.Handle_Vrml_TextureCoordinate2_IsNull(t)

def Handle_Vrml_TextureCoordinate2_ForceRelease(t):
    return _Vrml.Handle_Vrml_TextureCoordinate2_ForceRelease(t)

def Handle_Vrml_TextureCoordinate2_GetRefCount(t):
    return _Vrml.Handle_Vrml_TextureCoordinate2_GetRefCount(t)
class vrml(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    @staticmethod
    def CommentWriter(*args):
        r"""

        Parameters
        ----------
        aComment: str

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.vrml_CommentWriter(*args)

    @staticmethod
    def VrmlHeaderWriter(*args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        Writes a header in anOStream (VRML file). Writes one line of commentary in anOStream (VRML file).

        """
        return _Vrml.vrml_VrmlHeaderWriter(*args)

    __repr__ = _dumps_object


    def __init__(self):
        _Vrml.vrml_swiginit(self, _Vrml.new_vrml())
    __swig_destroy__ = _Vrml.delete_vrml

# Register vrml in _Vrml:
_Vrml.vrml_swigregister(vrml)
class Vrml_AsciiText(OCC.Core.Standard.Standard_Transient):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aString: TColStd_HArray1OfAsciiString
        aSpacing: float
        aJustification: Vrml_AsciiTextJustification
        aWidth: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_AsciiText_swiginit(self, _Vrml.new_Vrml_AsciiText(*args))

    def Justification(self, *args):
        r"""
        Return
        -------
        Vrml_AsciiTextJustification

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_AsciiText_Justification(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_AsciiText_Print(self, *args)

    def SetJustification(self, *args):
        r"""

        Parameters
        ----------
        aJustification: Vrml_AsciiTextJustification

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_AsciiText_SetJustification(self, *args)

    def SetSpacing(self, *args):
        r"""

        Parameters
        ----------
        aSpacing: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_AsciiText_SetSpacing(self, *args)

    def SetString(self, *args):
        r"""

        Parameters
        ----------
        aString: TColStd_HArray1OfAsciiString

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_AsciiText_SetString(self, *args)

    def SetWidth(self, *args):
        r"""

        Parameters
        ----------
        aWidth: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_AsciiText_SetWidth(self, *args)

    def Spacing(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_AsciiText_Spacing(self, *args)

    def String(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColStd_HArray1OfAsciiString>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_AsciiText_String(self, *args)

    def Width(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_AsciiText_Width(self, *args)


    @staticmethod
    def DownCast(t):
      return Handle_Vrml_AsciiText_DownCast(t)


    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_AsciiText

# Register Vrml_AsciiText in _Vrml:
_Vrml.Vrml_AsciiText_swigregister(Vrml_AsciiText)
class Vrml_Cone(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aParts: Vrml_ConeParts (optional, default to Vrml_ConeALL)
        aBottomRadius: float (optional, default to 1)
        aHeight: float (optional, default to 2)

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Cone_swiginit(self, _Vrml.new_Vrml_Cone(*args))

    def BottomRadius(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cone_BottomRadius(self, *args)

    def Height(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cone_Height(self, *args)

    def Parts(self, *args):
        r"""
        Return
        -------
        Vrml_ConeParts

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cone_Parts(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cone_Print(self, *args)

    def SetBottomRadius(self, *args):
        r"""

        Parameters
        ----------
        aBottomRadius: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cone_SetBottomRadius(self, *args)

    def SetHeight(self, *args):
        r"""

        Parameters
        ----------
        aHeight: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cone_SetHeight(self, *args)

    def SetParts(self, *args):
        r"""

        Parameters
        ----------
        aParts: Vrml_ConeParts

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cone_SetParts(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Cone

# Register Vrml_Cone in _Vrml:
_Vrml.Vrml_Cone_swigregister(Vrml_Cone)
class Vrml_Coordinate3(OCC.Core.Standard.Standard_Transient):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aPoint: TColgp_HArray1OfVec

        Return
        -------
        None

        Description
        -----------
        No available documentation.
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Coordinate3_swiginit(self, _Vrml.new_Vrml_Coordinate3(*args))

    def Point(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColgp_HArray1OfVec>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Coordinate3_Point(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Coordinate3_Print(self, *args)

    def SetPoint(self, *args):
        r"""

        Parameters
        ----------
        aPoint: TColgp_HArray1OfVec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Coordinate3_SetPoint(self, *args)


    @staticmethod
    def DownCast(t):
      return Handle_Vrml_Coordinate3_DownCast(t)


    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Coordinate3

# Register Vrml_Coordinate3 in _Vrml:
_Vrml.Vrml_Coordinate3_swigregister(Vrml_Coordinate3)
class Vrml_Cube(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aWidth: float (optional, default to 2)
        aHeight: float (optional, default to 2)
        aDepth: float (optional, default to 2)

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Cube_swiginit(self, _Vrml.new_Vrml_Cube(*args))

    def Depth(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cube_Depth(self, *args)

    def Height(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cube_Height(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cube_Print(self, *args)

    def SetDepth(self, *args):
        r"""

        Parameters
        ----------
        aDepth: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cube_SetDepth(self, *args)

    def SetHeight(self, *args):
        r"""

        Parameters
        ----------
        aHeight: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cube_SetHeight(self, *args)

    def SetWidth(self, *args):
        r"""

        Parameters
        ----------
        aWidth: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cube_SetWidth(self, *args)

    def Width(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cube_Width(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Cube

# Register Vrml_Cube in _Vrml:
_Vrml.Vrml_Cube_swigregister(Vrml_Cube)
class Vrml_Cylinder(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aParts: Vrml_CylinderParts (optional, default to Vrml_CylinderALL)
        aRadius: float (optional, default to 1)
        aHeight: float (optional, default to 2)

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Cylinder_swiginit(self, _Vrml.new_Vrml_Cylinder(*args))

    def Height(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cylinder_Height(self, *args)

    def Parts(self, *args):
        r"""
        Return
        -------
        Vrml_CylinderParts

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cylinder_Parts(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cylinder_Print(self, *args)

    def Radius(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cylinder_Radius(self, *args)

    def SetHeight(self, *args):
        r"""

        Parameters
        ----------
        aHeight: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cylinder_SetHeight(self, *args)

    def SetParts(self, *args):
        r"""

        Parameters
        ----------
        aParts: Vrml_CylinderParts

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cylinder_SetParts(self, *args)

    def SetRadius(self, *args):
        r"""

        Parameters
        ----------
        aRadius: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Cylinder_SetRadius(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Cylinder

# Register Vrml_Cylinder in _Vrml:
_Vrml.Vrml_Cylinder_swigregister(Vrml_Cylinder)
class Vrml_DirectionalLight(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aOnOff: bool
        aIntensity: float
        aColor: Quantity_Color
        aDirection: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_DirectionalLight_swiginit(self, _Vrml.new_Vrml_DirectionalLight(*args))

    def Color(self, *args):
        r"""
        Return
        -------
        Quantity_Color

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_DirectionalLight_Color(self, *args)

    def Direction(self, *args):
        r"""
        Return
        -------
        gp_Vec

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_DirectionalLight_Direction(self, *args)

    def Intensity(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_DirectionalLight_Intensity(self, *args)

    def OnOff(self, *args):
        r"""
        Return
        -------
        bool

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_DirectionalLight_OnOff(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_DirectionalLight_Print(self, *args)

    def SetColor(self, *args):
        r"""

        Parameters
        ----------
        aColor: Quantity_Color

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_DirectionalLight_SetColor(self, *args)

    def SetDirection(self, *args):
        r"""

        Parameters
        ----------
        aDirection: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_DirectionalLight_SetDirection(self, *args)

    def SetIntensity(self, *args):
        r"""

        Parameters
        ----------
        aIntensity: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_DirectionalLight_SetIntensity(self, *args)

    def SetOnOff(self, *args):
        r"""

        Parameters
        ----------
        aOnOff: bool

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_DirectionalLight_SetOnOff(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_DirectionalLight

# Register Vrml_DirectionalLight in _Vrml:
_Vrml.Vrml_DirectionalLight_swigregister(Vrml_DirectionalLight)
class Vrml_FontStyle(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aSize: float (optional, default to 10)
        aFamily: Vrml_FontStyleFamily (optional, default to Vrml_SERIF)
        aStyle: Vrml_FontStyleStyle (optional, default to Vrml_NONE)

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_FontStyle_swiginit(self, _Vrml.new_Vrml_FontStyle(*args))

    def Family(self, *args):
        r"""
        Return
        -------
        Vrml_FontStyleFamily

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_FontStyle_Family(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_FontStyle_Print(self, *args)

    def SetFamily(self, *args):
        r"""

        Parameters
        ----------
        aFamily: Vrml_FontStyleFamily

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_FontStyle_SetFamily(self, *args)

    def SetSize(self, *args):
        r"""

        Parameters
        ----------
        aSize: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_FontStyle_SetSize(self, *args)

    def SetStyle(self, *args):
        r"""

        Parameters
        ----------
        aStyle: Vrml_FontStyleStyle

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_FontStyle_SetStyle(self, *args)

    def Size(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_FontStyle_Size(self, *args)

    def Style(self, *args):
        r"""
        Return
        -------
        Vrml_FontStyleStyle

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_FontStyle_Style(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_FontStyle

# Register Vrml_FontStyle in _Vrml:
_Vrml.Vrml_FontStyle_swigregister(Vrml_FontStyle)
class Vrml_Group(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Group_swiginit(self, _Vrml.new_Vrml_Group(*args))

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Group_Print(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Group

# Register Vrml_Group in _Vrml:
_Vrml.Vrml_Group_swigregister(Vrml_Group)
class Vrml_IndexedFaceSet(OCC.Core.Standard.Standard_Transient):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aCoordIndex: TColStd_HArray1OfInteger
        aMaterialIndex: TColStd_HArray1OfInteger
        aNormalIndex: TColStd_HArray1OfInteger
        aTextureCoordIndex: TColStd_HArray1OfInteger

        Return
        -------
        None

        Description
        -----------
        No available documentation.
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_IndexedFaceSet_swiginit(self, _Vrml.new_Vrml_IndexedFaceSet(*args))

    def CoordIndex(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColStd_HArray1OfInteger>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedFaceSet_CoordIndex(self, *args)

    def MaterialIndex(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColStd_HArray1OfInteger>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedFaceSet_MaterialIndex(self, *args)

    def NormalIndex(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColStd_HArray1OfInteger>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedFaceSet_NormalIndex(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedFaceSet_Print(self, *args)

    def SetCoordIndex(self, *args):
        r"""

        Parameters
        ----------
        aCoordIndex: TColStd_HArray1OfInteger

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedFaceSet_SetCoordIndex(self, *args)

    def SetMaterialIndex(self, *args):
        r"""

        Parameters
        ----------
        aMaterialIndex: TColStd_HArray1OfInteger

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedFaceSet_SetMaterialIndex(self, *args)

    def SetNormalIndex(self, *args):
        r"""

        Parameters
        ----------
        aNormalIndex: TColStd_HArray1OfInteger

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedFaceSet_SetNormalIndex(self, *args)

    def SetTextureCoordIndex(self, *args):
        r"""

        Parameters
        ----------
        aTextureCoordIndex: TColStd_HArray1OfInteger

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedFaceSet_SetTextureCoordIndex(self, *args)

    def TextureCoordIndex(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColStd_HArray1OfInteger>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedFaceSet_TextureCoordIndex(self, *args)


    @staticmethod
    def DownCast(t):
      return Handle_Vrml_IndexedFaceSet_DownCast(t)


    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_IndexedFaceSet

# Register Vrml_IndexedFaceSet in _Vrml:
_Vrml.Vrml_IndexedFaceSet_swigregister(Vrml_IndexedFaceSet)
class Vrml_IndexedLineSet(OCC.Core.Standard.Standard_Transient):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aCoordIndex: TColStd_HArray1OfInteger
        aMaterialIndex: TColStd_HArray1OfInteger
        aNormalIndex: TColStd_HArray1OfInteger
        aTextureCoordIndex: TColStd_HArray1OfInteger

        Return
        -------
        None

        Description
        -----------
        No available documentation.
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_IndexedLineSet_swiginit(self, _Vrml.new_Vrml_IndexedLineSet(*args))

    def CoordIndex(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColStd_HArray1OfInteger>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedLineSet_CoordIndex(self, *args)

    def MaterialIndex(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColStd_HArray1OfInteger>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedLineSet_MaterialIndex(self, *args)

    def NormalIndex(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColStd_HArray1OfInteger>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedLineSet_NormalIndex(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedLineSet_Print(self, *args)

    def SetCoordIndex(self, *args):
        r"""

        Parameters
        ----------
        aCoordIndex: TColStd_HArray1OfInteger

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedLineSet_SetCoordIndex(self, *args)

    def SetMaterialIndex(self, *args):
        r"""

        Parameters
        ----------
        aMaterialIndex: TColStd_HArray1OfInteger

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedLineSet_SetMaterialIndex(self, *args)

    def SetNormalIndex(self, *args):
        r"""

        Parameters
        ----------
        aNormalIndex: TColStd_HArray1OfInteger

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedLineSet_SetNormalIndex(self, *args)

    def SetTextureCoordIndex(self, *args):
        r"""

        Parameters
        ----------
        aTextureCoordIndex: TColStd_HArray1OfInteger

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedLineSet_SetTextureCoordIndex(self, *args)

    def TextureCoordIndex(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColStd_HArray1OfInteger>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_IndexedLineSet_TextureCoordIndex(self, *args)


    @staticmethod
    def DownCast(t):
      return Handle_Vrml_IndexedLineSet_DownCast(t)


    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_IndexedLineSet

# Register Vrml_IndexedLineSet in _Vrml:
_Vrml.Vrml_IndexedLineSet_swigregister(Vrml_IndexedLineSet)
class Vrml_Info(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aString: str (optional, default to '<Undefinedinfo>')

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Info_swiginit(self, _Vrml.new_Vrml_Info(*args))

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Info_Print(self, *args)

    def SetString(self, *args):
        r"""

        Parameters
        ----------
        aString: str

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Info_SetString(self, *args)

    def String(self, *args):
        r"""
        Return
        -------
        TCollection_AsciiString

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Info_String(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Info

# Register Vrml_Info in _Vrml:
_Vrml.Vrml_Info_swigregister(Vrml_Info)
class Vrml_Instancing(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aString: str

        Return
        -------
        None

        Description
        -----------
        Adds 'DEF <myName>' in anOStream (VRML file).

        """
        _Vrml.Vrml_Instancing_swiginit(self, _Vrml.new_Vrml_Instancing(*args))

    def DEF(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        Adds 'USE <myName>' in anOStream (VRML file).

        """
        return _Vrml.Vrml_Instancing_DEF(self, *args)

    def USE(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Instancing_USE(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Instancing

# Register Vrml_Instancing in _Vrml:
_Vrml.Vrml_Instancing_swigregister(Vrml_Instancing)
class Vrml_LOD(OCC.Core.Standard.Standard_Transient):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aRange: TColStd_HArray1OfReal
        aCenter: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_LOD_swiginit(self, _Vrml.new_Vrml_LOD(*args))

    def Center(self, *args):
        r"""
        Return
        -------
        gp_Vec

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_LOD_Center(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_LOD_Print(self, *args)

    def Range(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColStd_HArray1OfReal>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_LOD_Range(self, *args)

    def SetCenter(self, *args):
        r"""

        Parameters
        ----------
        aCenter: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_LOD_SetCenter(self, *args)

    def SetRange(self, *args):
        r"""

        Parameters
        ----------
        aRange: TColStd_HArray1OfReal

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_LOD_SetRange(self, *args)


    @staticmethod
    def DownCast(t):
      return Handle_Vrml_LOD_DownCast(t)


    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_LOD

# Register Vrml_LOD in _Vrml:
_Vrml.Vrml_LOD_swigregister(Vrml_LOD)
class Vrml_Material(OCC.Core.Standard.Standard_Transient):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aAmbientColor: Quantity_HArray1OfColor
        aDiffuseColor: Quantity_HArray1OfColor
        aSpecularColor: Quantity_HArray1OfColor
        aEmissiveColor: Quantity_HArray1OfColor
        aShininess: TColStd_HArray1OfReal
        aTransparency: TColStd_HArray1OfReal

        Return
        -------
        None

        Description
        -----------
        No available documentation.
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Material_swiginit(self, _Vrml.new_Vrml_Material(*args))

    def AmbientColor(self, *args):
        r"""
        Return
        -------
        opencascade::handle<Quantity_HArray1OfColor>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Material_AmbientColor(self, *args)

    def DiffuseColor(self, *args):
        r"""
        Return
        -------
        opencascade::handle<Quantity_HArray1OfColor>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Material_DiffuseColor(self, *args)

    def EmissiveColor(self, *args):
        r"""
        Return
        -------
        opencascade::handle<Quantity_HArray1OfColor>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Material_EmissiveColor(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Material_Print(self, *args)

    def SetAmbientColor(self, *args):
        r"""

        Parameters
        ----------
        aAmbientColor: Quantity_HArray1OfColor

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Material_SetAmbientColor(self, *args)

    def SetDiffuseColor(self, *args):
        r"""

        Parameters
        ----------
        aDiffuseColor: Quantity_HArray1OfColor

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Material_SetDiffuseColor(self, *args)

    def SetEmissiveColor(self, *args):
        r"""

        Parameters
        ----------
        aEmissiveColor: Quantity_HArray1OfColor

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Material_SetEmissiveColor(self, *args)

    def SetShininess(self, *args):
        r"""

        Parameters
        ----------
        aShininess: TColStd_HArray1OfReal

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Material_SetShininess(self, *args)

    def SetSpecularColor(self, *args):
        r"""

        Parameters
        ----------
        aSpecularColor: Quantity_HArray1OfColor

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Material_SetSpecularColor(self, *args)

    def SetTransparency(self, *args):
        r"""

        Parameters
        ----------
        aTransparency: TColStd_HArray1OfReal

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Material_SetTransparency(self, *args)

    def Shininess(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColStd_HArray1OfReal>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Material_Shininess(self, *args)

    def SpecularColor(self, *args):
        r"""
        Return
        -------
        opencascade::handle<Quantity_HArray1OfColor>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Material_SpecularColor(self, *args)

    def Transparency(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColStd_HArray1OfReal>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Material_Transparency(self, *args)


    @staticmethod
    def DownCast(t):
      return Handle_Vrml_Material_DownCast(t)


    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Material

# Register Vrml_Material in _Vrml:
_Vrml.Vrml_Material_swigregister(Vrml_Material)
class Vrml_MaterialBinding(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aValue: Vrml_MaterialBindingAndNormalBinding

        Return
        -------
        None

        Description
        -----------
        No available documentation.
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_MaterialBinding_swiginit(self, _Vrml.new_Vrml_MaterialBinding(*args))

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_MaterialBinding_Print(self, *args)

    def SetValue(self, *args):
        r"""

        Parameters
        ----------
        aValue: Vrml_MaterialBindingAndNormalBinding

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_MaterialBinding_SetValue(self, *args)

    def Value(self, *args):
        r"""
        Return
        -------
        Vrml_MaterialBindingAndNormalBinding

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_MaterialBinding_Value(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_MaterialBinding

# Register Vrml_MaterialBinding in _Vrml:
_Vrml.Vrml_MaterialBinding_swigregister(Vrml_MaterialBinding)
class Vrml_MatrixTransform(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aMatrix: gp_Trsf

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_MatrixTransform_swiginit(self, _Vrml.new_Vrml_MatrixTransform(*args))

    def Matrix(self, *args):
        r"""
        Return
        -------
        gp_Trsf

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_MatrixTransform_Matrix(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_MatrixTransform_Print(self, *args)

    def SetMatrix(self, *args):
        r"""

        Parameters
        ----------
        aMatrix: gp_Trsf

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_MatrixTransform_SetMatrix(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_MatrixTransform

# Register Vrml_MatrixTransform in _Vrml:
_Vrml.Vrml_MatrixTransform_swigregister(Vrml_MatrixTransform)
class Vrml_Normal(OCC.Core.Standard.Standard_Transient):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aVector: TColgp_HArray1OfVec

        Return
        -------
        None

        Description
        -----------
        No available documentation.
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Normal_swiginit(self, _Vrml.new_Vrml_Normal(*args))

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Normal_Print(self, *args)

    def SetVector(self, *args):
        r"""

        Parameters
        ----------
        aVector: TColgp_HArray1OfVec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Normal_SetVector(self, *args)

    def Vector(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColgp_HArray1OfVec>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Normal_Vector(self, *args)


    @staticmethod
    def DownCast(t):
      return Handle_Vrml_Normal_DownCast(t)


    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Normal

# Register Vrml_Normal in _Vrml:
_Vrml.Vrml_Normal_swigregister(Vrml_Normal)
class Vrml_NormalBinding(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aValue: Vrml_MaterialBindingAndNormalBinding

        Return
        -------
        None

        Description
        -----------
        No available documentation.
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_NormalBinding_swiginit(self, _Vrml.new_Vrml_NormalBinding(*args))

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_NormalBinding_Print(self, *args)

    def SetValue(self, *args):
        r"""

        Parameters
        ----------
        aValue: Vrml_MaterialBindingAndNormalBinding

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_NormalBinding_SetValue(self, *args)

    def Value(self, *args):
        r"""
        Return
        -------
        Vrml_MaterialBindingAndNormalBinding

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_NormalBinding_Value(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_NormalBinding

# Register Vrml_NormalBinding in _Vrml:
_Vrml.Vrml_NormalBinding_swigregister(Vrml_NormalBinding)
class Vrml_OrthographicCamera(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aPosition: gp_Vec
        aOrientation: Vrml_SFRotation
        aFocalDistance: float
        aHeight: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_OrthographicCamera_swiginit(self, _Vrml.new_Vrml_OrthographicCamera(*args))

    def FocalDistance(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_OrthographicCamera_FocalDistance(self, *args)

    def Height(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_OrthographicCamera_Height(self, *args)

    def Orientation(self, *args):
        r"""
        Return
        -------
        Vrml_SFRotation

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_OrthographicCamera_Orientation(self, *args)

    def Position(self, *args):
        r"""
        Return
        -------
        gp_Vec

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_OrthographicCamera_Position(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_OrthographicCamera_Print(self, *args)

    def SetFocalDistance(self, *args):
        r"""

        Parameters
        ----------
        aFocalDistance: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_OrthographicCamera_SetFocalDistance(self, *args)

    def SetHeight(self, *args):
        r"""

        Parameters
        ----------
        aHeight: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_OrthographicCamera_SetHeight(self, *args)

    def SetOrientation(self, *args):
        r"""

        Parameters
        ----------
        aOrientation: Vrml_SFRotation

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_OrthographicCamera_SetOrientation(self, *args)

    def SetPosition(self, *args):
        r"""

        Parameters
        ----------
        aPosition: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_OrthographicCamera_SetPosition(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_OrthographicCamera

# Register Vrml_OrthographicCamera in _Vrml:
_Vrml.Vrml_OrthographicCamera_swigregister(Vrml_OrthographicCamera)
class Vrml_PerspectiveCamera(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aPosition: gp_Vec
        aOrientation: Vrml_SFRotation
        aFocalDistance: float
        aHeightAngle: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_PerspectiveCamera_swiginit(self, _Vrml.new_Vrml_PerspectiveCamera(*args))

    def Angle(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PerspectiveCamera_Angle(self, *args)

    def FocalDistance(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PerspectiveCamera_FocalDistance(self, *args)

    def Orientation(self, *args):
        r"""
        Return
        -------
        Vrml_SFRotation

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PerspectiveCamera_Orientation(self, *args)

    def Position(self, *args):
        r"""
        Return
        -------
        gp_Vec

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PerspectiveCamera_Position(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PerspectiveCamera_Print(self, *args)

    def SetAngle(self, *args):
        r"""

        Parameters
        ----------
        aHeightAngle: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PerspectiveCamera_SetAngle(self, *args)

    def SetFocalDistance(self, *args):
        r"""

        Parameters
        ----------
        aFocalDistance: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PerspectiveCamera_SetFocalDistance(self, *args)

    def SetOrientation(self, *args):
        r"""

        Parameters
        ----------
        aOrientation: Vrml_SFRotation

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PerspectiveCamera_SetOrientation(self, *args)

    def SetPosition(self, *args):
        r"""

        Parameters
        ----------
        aPosition: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PerspectiveCamera_SetPosition(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_PerspectiveCamera

# Register Vrml_PerspectiveCamera in _Vrml:
_Vrml.Vrml_PerspectiveCamera_swigregister(Vrml_PerspectiveCamera)
class Vrml_PointLight(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aOnOff: bool
        aIntensity: float
        aColor: Quantity_Color
        aLocation: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_PointLight_swiginit(self, _Vrml.new_Vrml_PointLight(*args))

    def Color(self, *args):
        r"""
        Return
        -------
        Quantity_Color

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PointLight_Color(self, *args)

    def Intensity(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PointLight_Intensity(self, *args)

    def Location(self, *args):
        r"""
        Return
        -------
        gp_Vec

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PointLight_Location(self, *args)

    def OnOff(self, *args):
        r"""
        Return
        -------
        bool

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PointLight_OnOff(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PointLight_Print(self, *args)

    def SetColor(self, *args):
        r"""

        Parameters
        ----------
        aColor: Quantity_Color

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PointLight_SetColor(self, *args)

    def SetIntensity(self, *args):
        r"""

        Parameters
        ----------
        aIntensity: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PointLight_SetIntensity(self, *args)

    def SetLocation(self, *args):
        r"""

        Parameters
        ----------
        aLocation: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PointLight_SetLocation(self, *args)

    def SetOnOff(self, *args):
        r"""

        Parameters
        ----------
        aOnOff: bool

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PointLight_SetOnOff(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_PointLight

# Register Vrml_PointLight in _Vrml:
_Vrml.Vrml_PointLight_swigregister(Vrml_PointLight)
class Vrml_PointSet(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aStartIndex: int (optional, default to 0)
        aNumPoints: int (optional, default to -1)

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_PointSet_swiginit(self, _Vrml.new_Vrml_PointSet(*args))

    def NumPoints(self, *args):
        r"""
        Return
        -------
        int

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PointSet_NumPoints(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PointSet_Print(self, *args)

    def SetNumPoints(self, *args):
        r"""

        Parameters
        ----------
        aNumPoints: int

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PointSet_SetNumPoints(self, *args)

    def SetStartIndex(self, *args):
        r"""

        Parameters
        ----------
        aStartIndex: int

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PointSet_SetStartIndex(self, *args)

    def StartIndex(self, *args):
        r"""
        Return
        -------
        int

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_PointSet_StartIndex(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_PointSet

# Register Vrml_PointSet in _Vrml:
_Vrml.Vrml_PointSet_swigregister(Vrml_PointSet)
class Vrml_Rotation(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aRotation: Vrml_SFRotation

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Rotation_swiginit(self, _Vrml.new_Vrml_Rotation(*args))

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Rotation_Print(self, *args)

    def Rotation(self, *args):
        r"""
        Return
        -------
        Vrml_SFRotation

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Rotation_Rotation(self, *args)

    def SetRotation(self, *args):
        r"""

        Parameters
        ----------
        aRotation: Vrml_SFRotation

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Rotation_SetRotation(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Rotation

# Register Vrml_Rotation in _Vrml:
_Vrml.Vrml_Rotation_swigregister(Vrml_Rotation)
class Vrml_SFImage(OCC.Core.Standard.Standard_Transient):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aWidth: int
        aHeight: int
        aNumber: Vrml_SFImageNumber
        anArray: TColStd_HArray1OfInteger

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_SFImage_swiginit(self, _Vrml.new_Vrml_SFImage(*args))

    def Array(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColStd_HArray1OfInteger>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFImage_Array(self, *args)

    def ArrayFlag(self, *args):
        r"""
        Return
        -------
        bool

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFImage_ArrayFlag(self, *args)

    def Height(self, *args):
        r"""
        Return
        -------
        int

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFImage_Height(self, *args)

    def Number(self, *args):
        r"""
        Return
        -------
        Vrml_SFImageNumber

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFImage_Number(self, *args)

    def SetArray(self, *args):
        r"""

        Parameters
        ----------
        anArray: TColStd_HArray1OfInteger

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFImage_SetArray(self, *args)

    def SetHeight(self, *args):
        r"""

        Parameters
        ----------
        aHeight: int

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFImage_SetHeight(self, *args)

    def SetNumber(self, *args):
        r"""

        Parameters
        ----------
        aNumber: Vrml_SFImageNumber

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFImage_SetNumber(self, *args)

    def SetWidth(self, *args):
        r"""

        Parameters
        ----------
        aWidth: int

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFImage_SetWidth(self, *args)

    def Width(self, *args):
        r"""
        Return
        -------
        int

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFImage_Width(self, *args)


    @staticmethod
    def DownCast(t):
      return Handle_Vrml_SFImage_DownCast(t)


    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_SFImage

# Register Vrml_SFImage in _Vrml:
_Vrml.Vrml_SFImage_swigregister(Vrml_SFImage)
class Vrml_SFRotation(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aRotationX: float
        aRotationY: float
        aRotationZ: float
        anAngle: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_SFRotation_swiginit(self, _Vrml.new_Vrml_SFRotation(*args))

    def Angle(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFRotation_Angle(self, *args)

    def RotationX(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFRotation_RotationX(self, *args)

    def RotationY(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFRotation_RotationY(self, *args)

    def RotationZ(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFRotation_RotationZ(self, *args)

    def SetAngle(self, *args):
        r"""

        Parameters
        ----------
        anAngle: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFRotation_SetAngle(self, *args)

    def SetRotationX(self, *args):
        r"""

        Parameters
        ----------
        aRotationX: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFRotation_SetRotationX(self, *args)

    def SetRotationY(self, *args):
        r"""

        Parameters
        ----------
        aRotationY: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFRotation_SetRotationY(self, *args)

    def SetRotationZ(self, *args):
        r"""

        Parameters
        ----------
        aRotationZ: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SFRotation_SetRotationZ(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_SFRotation

# Register Vrml_SFRotation in _Vrml:
_Vrml.Vrml_SFRotation_swigregister(Vrml_SFRotation)
class Vrml_Scale(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aScaleFactor: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Scale_swiginit(self, _Vrml.new_Vrml_Scale(*args))

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Scale_Print(self, *args)

    def ScaleFactor(self, *args):
        r"""
        Return
        -------
        gp_Vec

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Scale_ScaleFactor(self, *args)

    def SetScaleFactor(self, *args):
        r"""

        Parameters
        ----------
        aScaleFactor: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Scale_SetScaleFactor(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Scale

# Register Vrml_Scale in _Vrml:
_Vrml.Vrml_Scale_swigregister(Vrml_Scale)
class Vrml_Separator(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aRenderCulling: Vrml_SeparatorRenderCulling

        Return
        -------
        None

        Description
        -----------
        No available documentation.
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Separator_swiginit(self, _Vrml.new_Vrml_Separator(*args))

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Separator_Print(self, *args)

    def RenderCulling(self, *args):
        r"""
        Return
        -------
        Vrml_SeparatorRenderCulling

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Separator_RenderCulling(self, *args)

    def SetRenderCulling(self, *args):
        r"""

        Parameters
        ----------
        aRenderCulling: Vrml_SeparatorRenderCulling

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Separator_SetRenderCulling(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Separator

# Register Vrml_Separator in _Vrml:
_Vrml.Vrml_Separator_swigregister(Vrml_Separator)
class Vrml_ShapeHints(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aVertexOrdering: Vrml_VertexOrdering (optional, default to Vrml_UNKNOWN_ORDERING)
        aShapeType: Vrml_ShapeType (optional, default to Vrml_UNKNOWN_SHAPE_TYPE)
        aFaceType: Vrml_FaceType (optional, default to Vrml_CONVEX)
        aAngle: float (optional, default to 0.5)

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_ShapeHints_swiginit(self, _Vrml.new_Vrml_ShapeHints(*args))

    def Angle(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_ShapeHints_Angle(self, *args)

    def FaceType(self, *args):
        r"""
        Return
        -------
        Vrml_FaceType

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_ShapeHints_FaceType(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_ShapeHints_Print(self, *args)

    def SetAngle(self, *args):
        r"""

        Parameters
        ----------
        aAngle: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_ShapeHints_SetAngle(self, *args)

    def SetFaceType(self, *args):
        r"""

        Parameters
        ----------
        aFaceType: Vrml_FaceType

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_ShapeHints_SetFaceType(self, *args)

    def SetShapeType(self, *args):
        r"""

        Parameters
        ----------
        aShapeType: Vrml_ShapeType

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_ShapeHints_SetShapeType(self, *args)

    def SetVertexOrdering(self, *args):
        r"""

        Parameters
        ----------
        aVertexOrdering: Vrml_VertexOrdering

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_ShapeHints_SetVertexOrdering(self, *args)

    def ShapeType(self, *args):
        r"""
        Return
        -------
        Vrml_ShapeType

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_ShapeHints_ShapeType(self, *args)

    def VertexOrdering(self, *args):
        r"""
        Return
        -------
        Vrml_VertexOrdering

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_ShapeHints_VertexOrdering(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_ShapeHints

# Register Vrml_ShapeHints in _Vrml:
_Vrml.Vrml_ShapeHints_swigregister(Vrml_ShapeHints)
class Vrml_Sphere(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aRadius: float (optional, default to 1)

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Sphere_swiginit(self, _Vrml.new_Vrml_Sphere(*args))

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Sphere_Print(self, *args)

    def Radius(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Sphere_Radius(self, *args)

    def SetRadius(self, *args):
        r"""

        Parameters
        ----------
        aRadius: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Sphere_SetRadius(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Sphere

# Register Vrml_Sphere in _Vrml:
_Vrml.Vrml_Sphere_swigregister(Vrml_Sphere)
class Vrml_SpotLight(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aOnOff: bool
        aIntensity: float
        aColor: Quantity_Color
        aLocation: gp_Vec
        aDirection: gp_Vec
        aDropOffRate: float
        aCutOffAngle: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_SpotLight_swiginit(self, _Vrml.new_Vrml_SpotLight(*args))

    def Color(self, *args):
        r"""
        Return
        -------
        Quantity_Color

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_Color(self, *args)

    def CutOffAngle(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_CutOffAngle(self, *args)

    def Direction(self, *args):
        r"""
        Return
        -------
        gp_Vec

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_Direction(self, *args)

    def DropOffRate(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_DropOffRate(self, *args)

    def Intensity(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_Intensity(self, *args)

    def Location(self, *args):
        r"""
        Return
        -------
        gp_Vec

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_Location(self, *args)

    def OnOff(self, *args):
        r"""
        Return
        -------
        bool

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_OnOff(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_Print(self, *args)

    def SetColor(self, *args):
        r"""

        Parameters
        ----------
        aColor: Quantity_Color

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_SetColor(self, *args)

    def SetCutOffAngle(self, *args):
        r"""

        Parameters
        ----------
        aCutOffAngle: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_SetCutOffAngle(self, *args)

    def SetDirection(self, *args):
        r"""

        Parameters
        ----------
        aDirection: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_SetDirection(self, *args)

    def SetDropOffRate(self, *args):
        r"""

        Parameters
        ----------
        aDropOffRate: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_SetDropOffRate(self, *args)

    def SetIntensity(self, *args):
        r"""

        Parameters
        ----------
        aIntensity: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_SetIntensity(self, *args)

    def SetLocation(self, *args):
        r"""

        Parameters
        ----------
        aLocation: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_SetLocation(self, *args)

    def SetOnOff(self, *args):
        r"""

        Parameters
        ----------
        anOnOff: bool

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_SpotLight_SetOnOff(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_SpotLight

# Register Vrml_SpotLight in _Vrml:
_Vrml.Vrml_SpotLight_swigregister(Vrml_SpotLight)
class Vrml_Switch(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aWhichChild: int (optional, default to -1)

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Switch_swiginit(self, _Vrml.new_Vrml_Switch(*args))

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Switch_Print(self, *args)

    def SetWhichChild(self, *args):
        r"""

        Parameters
        ----------
        aWhichChild: int

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Switch_SetWhichChild(self, *args)

    def WhichChild(self, *args):
        r"""
        Return
        -------
        int

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Switch_WhichChild(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Switch

# Register Vrml_Switch in _Vrml:
_Vrml.Vrml_Switch_swigregister(Vrml_Switch)
class Vrml_Texture2(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aFilename: str
        aImage: Vrml_SFImage
        aWrapS: Vrml_Texture2Wrap
        aWrapT: Vrml_Texture2Wrap

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Texture2_swiginit(self, _Vrml.new_Vrml_Texture2(*args))

    def Filename(self, *args):
        r"""
        Return
        -------
        TCollection_AsciiString

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2_Filename(self, *args)

    def Image(self, *args):
        r"""
        Return
        -------
        opencascade::handle<Vrml_SFImage>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2_Image(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2_Print(self, *args)

    def SetFilename(self, *args):
        r"""

        Parameters
        ----------
        aFilename: str

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2_SetFilename(self, *args)

    def SetImage(self, *args):
        r"""

        Parameters
        ----------
        aImage: Vrml_SFImage

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2_SetImage(self, *args)

    def SetWrapS(self, *args):
        r"""

        Parameters
        ----------
        aWrapS: Vrml_Texture2Wrap

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2_SetWrapS(self, *args)

    def SetWrapT(self, *args):
        r"""

        Parameters
        ----------
        aWrapT: Vrml_Texture2Wrap

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2_SetWrapT(self, *args)

    def WrapS(self, *args):
        r"""
        Return
        -------
        Vrml_Texture2Wrap

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2_WrapS(self, *args)

    def WrapT(self, *args):
        r"""
        Return
        -------
        Vrml_Texture2Wrap

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2_WrapT(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Texture2

# Register Vrml_Texture2 in _Vrml:
_Vrml.Vrml_Texture2_swigregister(Vrml_Texture2)
class Vrml_Texture2Transform(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aTranslation: gp_Vec2d
        aRotation: float
        aScaleFactor: gp_Vec2d
        aCenter: gp_Vec2d

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Texture2Transform_swiginit(self, _Vrml.new_Vrml_Texture2Transform(*args))

    def Center(self, *args):
        r"""
        Return
        -------
        gp_Vec2d

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2Transform_Center(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2Transform_Print(self, *args)

    def Rotation(self, *args):
        r"""
        Return
        -------
        float

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2Transform_Rotation(self, *args)

    def ScaleFactor(self, *args):
        r"""
        Return
        -------
        gp_Vec2d

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2Transform_ScaleFactor(self, *args)

    def SetCenter(self, *args):
        r"""

        Parameters
        ----------
        aCenter: gp_Vec2d

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2Transform_SetCenter(self, *args)

    def SetRotation(self, *args):
        r"""

        Parameters
        ----------
        aRotation: float

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2Transform_SetRotation(self, *args)

    def SetScaleFactor(self, *args):
        r"""

        Parameters
        ----------
        aScaleFactor: gp_Vec2d

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2Transform_SetScaleFactor(self, *args)

    def SetTranslation(self, *args):
        r"""

        Parameters
        ----------
        aTranslation: gp_Vec2d

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2Transform_SetTranslation(self, *args)

    def Translation(self, *args):
        r"""
        Return
        -------
        gp_Vec2d

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Texture2Transform_Translation(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Texture2Transform

# Register Vrml_Texture2Transform in _Vrml:
_Vrml.Vrml_Texture2Transform_swigregister(Vrml_Texture2Transform)
class Vrml_TextureCoordinate2(OCC.Core.Standard.Standard_Transient):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aPoint: TColgp_HArray1OfVec2d

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_TextureCoordinate2_swiginit(self, _Vrml.new_Vrml_TextureCoordinate2(*args))

    def Point(self, *args):
        r"""
        Return
        -------
        opencascade::handle<TColgp_HArray1OfVec2d>

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_TextureCoordinate2_Point(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_TextureCoordinate2_Print(self, *args)

    def SetPoint(self, *args):
        r"""

        Parameters
        ----------
        aPoint: TColgp_HArray1OfVec2d

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_TextureCoordinate2_SetPoint(self, *args)


    @staticmethod
    def DownCast(t):
      return Handle_Vrml_TextureCoordinate2_DownCast(t)


    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_TextureCoordinate2

# Register Vrml_TextureCoordinate2 in _Vrml:
_Vrml.Vrml_TextureCoordinate2_swigregister(Vrml_TextureCoordinate2)
class Vrml_Transform(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aTranslation: gp_Vec
        aRotation: Vrml_SFRotation
        aScaleFactor: gp_Vec
        aScaleOrientation: Vrml_SFRotation
        aCenter: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Transform_swiginit(self, _Vrml.new_Vrml_Transform(*args))

    def Center(self, *args):
        r"""
        Return
        -------
        gp_Vec

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Transform_Center(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Transform_Print(self, *args)

    def Rotation(self, *args):
        r"""
        Return
        -------
        Vrml_SFRotation

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Transform_Rotation(self, *args)

    def ScaleFactor(self, *args):
        r"""
        Return
        -------
        gp_Vec

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Transform_ScaleFactor(self, *args)

    def ScaleOrientation(self, *args):
        r"""
        Return
        -------
        Vrml_SFRotation

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Transform_ScaleOrientation(self, *args)

    def SetCenter(self, *args):
        r"""

        Parameters
        ----------
        aCenter: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Transform_SetCenter(self, *args)

    def SetRotation(self, *args):
        r"""

        Parameters
        ----------
        aRotation: Vrml_SFRotation

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Transform_SetRotation(self, *args)

    def SetScaleFactor(self, *args):
        r"""

        Parameters
        ----------
        aScaleFactor: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Transform_SetScaleFactor(self, *args)

    def SetScaleOrientation(self, *args):
        r"""

        Parameters
        ----------
        aScaleOrientation: Vrml_SFRotation

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Transform_SetScaleOrientation(self, *args)

    def SetTranslation(self, *args):
        r"""

        Parameters
        ----------
        aTranslation: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Transform_SetTranslation(self, *args)

    def Translation(self, *args):
        r"""
        Return
        -------
        gp_Vec

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Transform_Translation(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Transform

# Register Vrml_Transform in _Vrml:
_Vrml.Vrml_Transform_swigregister(Vrml_Transform)
class Vrml_TransformSeparator(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_TransformSeparator_swiginit(self, _Vrml.new_Vrml_TransformSeparator(*args))

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_TransformSeparator_Print(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_TransformSeparator

# Register Vrml_TransformSeparator in _Vrml:
_Vrml.Vrml_TransformSeparator_swigregister(Vrml_TransformSeparator)
class Vrml_Translation(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aTranslation: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_Translation_swiginit(self, _Vrml.new_Vrml_Translation(*args))

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Translation_Print(self, *args)

    def SetTranslation(self, *args):
        r"""

        Parameters
        ----------
        aTranslation: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Translation_SetTranslation(self, *args)

    def Translation(self, *args):
        r"""
        Return
        -------
        gp_Vec

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_Translation_Translation(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_Translation

# Register Vrml_Translation in _Vrml:
_Vrml.Vrml_Translation_swigregister(Vrml_Translation)
class Vrml_WWWAnchor(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""

        Parameters
        ----------
        aName: str (optional, default to )
        aDescription: str (optional, default to )
        aMap: Vrml_WWWAnchorMap (optional, default to Vrml_MAP_NONE)

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_WWWAnchor_swiginit(self, _Vrml.new_Vrml_WWWAnchor(*args))

    def Description(self, *args):
        r"""
        Return
        -------
        TCollection_AsciiString

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_WWWAnchor_Description(self, *args)

    def Map(self, *args):
        r"""
        Return
        -------
        Vrml_WWWAnchorMap

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_WWWAnchor_Map(self, *args)

    def Name(self, *args):
        r"""
        Return
        -------
        TCollection_AsciiString

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_WWWAnchor_Name(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_WWWAnchor_Print(self, *args)

    def SetDescription(self, *args):
        r"""

        Parameters
        ----------
        aDescription: str

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_WWWAnchor_SetDescription(self, *args)

    def SetMap(self, *args):
        r"""

        Parameters
        ----------
        aMap: Vrml_WWWAnchorMap

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_WWWAnchor_SetMap(self, *args)

    def SetName(self, *args):
        r"""

        Parameters
        ----------
        aName: str

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_WWWAnchor_SetName(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_WWWAnchor

# Register Vrml_WWWAnchor in _Vrml:
_Vrml.Vrml_WWWAnchor_swigregister(Vrml_WWWAnchor)
class Vrml_WWWInline(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        r"""
        Return
        -------
        None

        Description
        -----------
        No available documentation.

        Parameters
        ----------
        aName: str
        aBboxSize: gp_Vec
        aBboxCenter: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        _Vrml.Vrml_WWWInline_swiginit(self, _Vrml.new_Vrml_WWWInline(*args))

    def BboxCenter(self, *args):
        r"""
        Return
        -------
        gp_Vec

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_WWWInline_BboxCenter(self, *args)

    def BboxSize(self, *args):
        r"""
        Return
        -------
        gp_Vec

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_WWWInline_BboxSize(self, *args)

    def Name(self, *args):
        r"""
        Return
        -------
        TCollection_AsciiString

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_WWWInline_Name(self, *args)

    def Print(self, *args):
        r"""

        Parameters
        ----------

        Return
        -------
        anOStream: Standard_OStream

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_WWWInline_Print(self, *args)

    def SetBboxCenter(self, *args):
        r"""

        Parameters
        ----------
        aBboxCenter: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_WWWInline_SetBboxCenter(self, *args)

    def SetBboxSize(self, *args):
        r"""

        Parameters
        ----------
        aBboxSize: gp_Vec

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_WWWInline_SetBboxSize(self, *args)

    def SetName(self, *args):
        r"""

        Parameters
        ----------
        aName: str

        Return
        -------
        None

        Description
        -----------
        No available documentation.

        """
        return _Vrml.Vrml_WWWInline_SetName(self, *args)

    __repr__ = _dumps_object

    __swig_destroy__ = _Vrml.delete_Vrml_WWWInline

# Register Vrml_WWWInline in _Vrml:
_Vrml.Vrml_WWWInline_swigregister(Vrml_WWWInline)



@deprecated
def vrml_CommentWriter(*args):
	return vrml.CommentWriter(*args)

@deprecated
def vrml_VrmlHeaderWriter(*args):
	return vrml.VrmlHeaderWriter(*args)



