...
https://dev.opencascade.org/doc/occt-7.9.0/refman/html/package_xmlldrivers.html
"""
...
PYTHONOCC_DEBUG_EXCEPTIONS = _XmlLDrivers.PYTHONOCC_DEBUG_EXCEPTIONS

def get_readable_class_name(class_name):
    return _XmlLDrivers.get_readable_class_name(class_name)

def get_readable_method_name(method_name):
    return _XmlLDrivers.get_readable_method_name(method_name)

def get_exception_type(error):
    return _XmlLDrivers.get_exception_type(error)

def process_opencascade_exception(error, method_name, class_name):
    return _XmlLDrivers.process_opencascade_exception(error, method_name, class_name)
...
def Handle_XmlLDrivers_DocumentRetrievalDriver_ForceRelease(t):
    return _XmlLDrivers.Handle_XmlLDrivers_DocumentRetrievalDriver_ForceRelease(t)

def Handle_XmlLDrivers_DocumentRetrievalDriver_GetRefCount(t):
    return _XmlLDrivers.Handle_XmlLDrivers_DocumentRetrievalDriver_GetRefCount(t)
...
def Handle_XmlLDrivers_DocumentStorageDriver_ForceRelease(t):
    return _XmlLDrivers.Handle_XmlLDrivers_DocumentStorageDriver_ForceRelease(t)

def Handle_XmlLDrivers_DocumentStorageDriver_GetRefCount(t):
    return _XmlLDrivers.Handle_XmlLDrivers_DocumentStorageDriver_GetRefCount(t)
