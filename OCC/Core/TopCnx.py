...
https://dev.opencascade.org/doc/occt-7.9.0/refman/html/package_topcnx.html
"""
...
PYTHONOCC_DEBUG_EXCEPTIONS = _TopCnx.PYTHONOCC_DEBUG_EXCEPTIONS

def get_readable_class_name(class_name):
    return _TopCnx.get_readable_class_name(class_name)

def get_readable_method_name(method_name):
    return _TopCnx.get_readable_method_name(method_name)

def get_exception_type(error):
    return _TopCnx.get_exception_type(error)

def process_opencascade_exception(error, method_name, class_name):
    return _TopCnx.process_opencascade_exception(error, method_name, class_name)
