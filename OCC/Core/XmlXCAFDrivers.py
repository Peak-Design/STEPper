...
https://dev.opencascade.org/doc/occt-7.9.0/refman/html/package_xmlxcafdrivers.html
"""
...
PYTHONOCC_DEBUG_EXCEPTIONS = _XmlXCAFDrivers.PYTHONOCC_DEBUG_EXCEPTIONS

def get_readable_class_name(class_name):
    return _XmlXCAFDrivers.get_readable_class_name(class_name)

def get_readable_method_name(method_name):
    return _XmlXCAFDrivers.get_readable_method_name(method_name)

def get_exception_type(error):
    return _XmlXCAFDrivers.get_exception_type(error)

def process_opencascade_exception(error, method_name, class_name):
    return _XmlXCAFDrivers.process_opencascade_exception(error, method_name, class_name)
...
def Handle_XmlXCAFDrivers_DocumentRetrievalDriver_ForceRelease(t):
    return _XmlXCAFDrivers.Handle_XmlXCAFDrivers_DocumentRetrievalDriver_ForceRelease(t)

def Handle_XmlXCAFDrivers_DocumentRetrievalDriver_GetRefCount(t):
    return _XmlXCAFDrivers.Handle_XmlXCAFDrivers_DocumentRetrievalDriver_GetRefCount(t)
...
def Handle_XmlXCAFDrivers_DocumentStorageDriver_ForceRelease(t):
    return _XmlXCAFDrivers.Handle_XmlXCAFDrivers_DocumentStorageDriver_ForceRelease(t)

def Handle_XmlXCAFDrivers_DocumentStorageDriver_GetRefCount(t):
    return _XmlXCAFDrivers.Handle_XmlXCAFDrivers_DocumentStorageDriver_GetRefCount(t)
